...
var _data = _interopRequireDefault(
  require('caniuse-db/fulldata-json/data-2.0.json')
);
...
function formatTargetNames(target) {
  const name =
    _constants.STANDARD_TARGET_NAME_MAPPING[target.target] || target.target;
...
function isSupportedByCanIUse(node, { version, target, parsedVersion }) {
  ...
  if (typeof version === 'string' && versionIsRange(version)) {
    return Object.keys(targetStats).some((statsVersion) =>
